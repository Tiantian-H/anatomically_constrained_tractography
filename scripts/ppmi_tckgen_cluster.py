...
wmfods=${PREFIXFOD}_WMFODs_${rf_algo}.mif
wmfods_sft=${PREFIXFOD}_WMFODs_${rf_algo}_downsampled.mif
...
if [ ! -e ${wmfods}  ]; then
    ...
    if [ ! -e ${rfvoxels}  ]; then
        echo "dwi2response"
        dwi2response ${rf_algo} ${dwi} ${rf} -voxels ${rfvoxels}
        echo "    dwi2response complete"
    fi
    echo "dwi2fod"
    dwi2fod -mask ${dwimask} msmt_csd ${dwi} ${rf} ${wmfods}
...
def main():
    ...
    Modify as desired: 
      ...
      workingDir
    """
    workingDir = '/SAN/medic/Net_Mod_MS/cjd/ppmi_sMRI/act'
